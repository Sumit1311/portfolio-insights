    ...
    return userProfileDAO.getClient()
		...
		if(a.transactionDate == b.transactionDate) {
            ...
            for(var i = 0; i < parsedExcel.length; i++){
                if(validationResult[parsedExcel[i].securityCode] == undefined) {
                    validationResult[parsedExcel[i].securityCode] = 0;
                }
                    if(parsedExcel[i].transactionType == "Buy") {
                        validationResult[parsedExcel[i].securityCode] += parsedExcel[i].numberOfShares;
                    } else {
                        validationResult[parsedExcel[i].securityCode] -= parsedExcel[i].numberOfShares;
                    }
                    if(validationResult[parsedExcel[i].securityCode] < 0) {
                        throw new navValidationException("Invalid data for script : " + parsedExcel[i].securityCode);
                    }
