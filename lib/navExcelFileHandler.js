...
module.exports = class navExcelFileHandler {
    ...
    parseCellValue(columnName, columnData, rowNumber){
        ...
        deleteFromPortFolio(userId, transactionId){
            ...
            var transaction;
            new navUserProfileDAO().getTransaction(transactionId, userId)
                .then((data) => {
                    if(data.length == 0){
                    ...
                    var promise;
                        if (transaction.trxn_flag == 0) {
                            promise = new navUserProfileDAO().deleteTransaction(userId, transactionId);
                        } else if (transaction.trxn_flag == 1) {
                            promise = new navUserProfileDAO().markTransactionInactive(userId, transactionId);
                        } else {
