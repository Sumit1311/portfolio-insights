...
module.exports = class navExcelFileHandler {
    ...
    return userProfileDAO.getClient()
        ...
        .then(function(results){
            ...
            if(shouldMerge) {
                return self.validateDatabasePortfolio(userId);
            } else {
                return Q.resolve();
            }
    ...
    validateDatabasePortfolio(userId){
        ...
        deleteFromPortFolio(userId, transactionId){
            ...
            /*return da.getClient()
                ...
                .catch(
                    function (error) {
                        //logg error
                        navLogUtil.instance().log.call(self,self.deleteFromPortFolio.name, 'Error while deleting from portfolio' + error, "error");
                        return da.rollBackTx()
                            ...
                            .catch(function (err) {
                                //log error
                                navLogUtil.instance().log.call(self,self.deleteFromPortFolio.name, 'Error while deleting from portfolio' + err, "error");
                                return Q.reject(err)
                            })
                    })
                .finally(function () {
    ...
    undoFromPortFolio(userId, transactionId){
        var transaction;
        return new navUserProfileDAO().getTransaction(transactionId, userId)
            .then((data) => {
                if(data.length == 0){
                    throw new navValidationException("Transaction id not valid");
                }
                ...
                if (transaction.trxn_flag == 1) {
                    return new navUserProfileDAO().deleteTransaction(userId, transactionId);
                } else {
                    throw new Error("Something went wrong");
                }
            })
