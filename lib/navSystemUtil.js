...
module.exports = class navSystemUtils {
    ...
    runDailyScheduler (workPacket) {
        return this.executeCommand(navConfigParser.instance().getConfig("PythonExecutable", "python3") + " scripts/nav_calling_block.py dpv "+ workPacket.user._id, {
            shell : true,
            env : process.env
        })
        .then(function(result){
    ...
    runProfileRefresher (workPacket) {
        return this.executeCommand(navConfigParser.instance().getConfig("PythonExecutable", "python3") + " scripts/nav_calling_block.py rpv "+ workPacket.user._id, {
            shell : true,
            env : process.env
        })
        .then(function(result){
