...
module.exports = class navSystemUtils {
    ...
    runDailyScheduler (userId) {
        var self = this;
        return this.executeCommand(navConfigParser.instance().getConfig("PythonExecutable", "python3") + " scripts/nav_calling_block.py dpv "+ userId, {
            shell : true,
            env : process.env
        })
        .then(function(result){
    ...
    runProfileRefresher (userId) {
        var self = this;
        return this.executeCommand(navConfigParser.instance().getConfig("PythonExecutable", "python3") + " scripts/nav_calling_block.py rpv "+ userId, {
            shell : true,
            env : process.env
        })
        .then(function(result){
    ...
    runFullProfileRefresher (userId) {
        var self = this;
        return this.executeCommand(navConfigParser.instance().getConfig("PythonExecutable", "python3") + " scripts/nav_calling_block.py crpv "+ userId, {
            shell : true,
            env : process.env
        })
            .then(function(result){
