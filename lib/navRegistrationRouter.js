...
module.exports = class navRegistration extends navBaseRouter {
    ...
    doRegistration(req, res){
        var firstName = req.body.firstName, lastName = req.body.lastName, email = req.body.email, password = req.body.password, contactNo = req.body.mobileNo, passwordConf = req.body.passwordConf;
        ...
        deferred.promise
            .done(function(){
                ...
                res.render('success-page', {
                    ...
                    successMessage : 'Registration Success. Please verify your email address by going to your mailbox.'
                });
        },function(error){
        ...
        //req.assert("agree","Please agree the terms and conditions").isEnabled();
        ...
        .then(function(){
            ...
            navLogUtil.instance().log.call(self, self.doRegistration.name, "Verification link for user : " + email + " "+ verificationLink, "info");
             ...
             return emailVer.sendVerificationEmail(email, firstName +" "+ lastName, verificationLink)
        })
        .then(function (response) {
             navLogUtil.instance().log.call(self, self.doRegistration.name," Sent verification email" + response, 'info');
    ...
    doEmailVerification(req, res ) {
        ...
        deferred.promise
            .done((user) => {
                res.render('auth/signup/registration-page',{
                    layout : "modular-auth-layout",
                    isLoggedIn : true,
                    user : user,
    ...
    saveAdditionalDetails(req,res) {
        ...
        deferred.promise
            .done(() => {
                ...
                res.render('success-page', {
                    ...
                    themeTitle : 'Portfolio-Insights',
                    showLoginBtn : true,
                    successMessage : 'Verification Success. Please login to get access to Portfolio-Insights.'
                });
            },(error) => {
                var respUtil =  new navResponseUtil();
                var response = respUtil.generateErrorResponse(error);
        ...
        req.assert("mobileNo","Phone Number is Required").notEmpty();
    ...
    getResetPasswordInfo(req, res) {
        res.render('auth/reset/reset-page',{
            layout : 'modular-auth-layout',
            isLoggedIn : req.user ? true : false,
            title : 'Portfolio-Insights',
            themeTitle : 'Portfolio-Insights'
    ...
    resetPassword(req, res) {
        ...
        deferred.promise
            .done(function(){
                ...
                res.render('success-page', {
                    ...
                    themeTitle : 'Portfolio-Insights',
                    showLoginBtn : true,
                    successMessage : 'Reset Password link has been sent to your registered email address. Please reset your password using that link.'
                });
        },function(error){
                var response = respUtil.generateErrorResponse(error);
                respUtil.renderErrorPage(req, res, {
        ...
        new navAccount().getUserDetails(email)
        .then(function(user){
            if(user.length !== 0) {
            var emailVer = new navEmailVerification();
            verificationCode = emailVer.generateCode();
            ...
            base.pathname = "/reset";
            base.search = "?id=" + verificationCode;
            var resetPasswordLink = base.format();
            navLogUtil.instance().log.call(self, self.resetPassword.name, "Reset Password link for user : " + email + " "+ resetPasswordLink, "info");

             //return userDAO.insertRegistrationData(email, contactNo, password,verificationCode);
             //todo : uncomment when want to send verification email
             return emailVer.sendResetPassword(email, user[0].first_name + " " + user[0].last_name, resetPasswordLink)
            } else {
    ...
    getResetPassword(req, res) {
        ...
        deferred.promise
            .done((user) => {
                res.render('auth/reset/reset-password-page',{
                    layout : "modular-auth-layout",
                    isLoggedIn : false,
                    user : user,
        ...
        deferred.promise
            .done(() => {
                ...
                res.render('success-page', {
                    ...
                    themeTitle : 'Portfolio-Insights',
                    showLoginBtn : true,
                    successMessage : 'Password has been reset successfully. Please use your new password to login.'
                });
            },(error) => {
                var respUtil =  new navResponseUtil();
                var response = respUtil.generateErrorResponse(error);
    ...
    doSignup(req,res) {
        res.render('auth/signup/signup-page', {
            layout : 'modular-auth-layout',
            title : 'Portfolio-Insights',
            themeTitle : 'Portfolio-Insights'
        });
