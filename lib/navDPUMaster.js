...
module.exports = class navDPUMaster{
    ...
    start(){
        ...
        return userDAO.getUsersForDailyUpdate(1, parseInt(navConfigParser.instance().getConfig("StaleInterval", "600000")))
            .then(function(users){
...
parseInt(navConfigParser.instance().getConfig("StaleInterval", "600000") ))
            })
            .then((res) => {
    ...
    successHandler(workPacket) {
        ...
        new navUserDAO().unlockUserForDailyUpdate(userId, parseInt(navConfigParser.instance().getConfig("RefresherInterval", "600000")))
        .catch((error) => {
            return Q.reject(error);
        })
        .finally(() =>{
    ...
    errorHandler(workPacket) {
        ...
        new navUserDAO().unlockUserForDailyUpdate(userId, 0)
         .then(() => {
            return self.start();
         })
         .catch((error) => {
