...
UserDAO.prototype.getUsersForDailyUpdate = function(count){
    return this.dbQuery("SELECT * " +
            "FROM " + tableName +
        " WHERE ( next_execution_time IS NULL OR next_execution_time <= $1)"+
        " AND time_taken IS NULL LIMIT $2", 
        [new navCommonUtil().getCurrentTime(), count])
        .then(function (result) {
            return result.rows;
        })
        .catch(function (error) {
...
UserDAO.prototype.lockUserForDailyUpdate = function(userId){
    var t = new navCommonUtil().getCurrentTime();
    return this.dbQuery("UPDATE "+ tableName+" SET time_taken = $1 WHERE _id=$2 AND " +
    "( next_execution_time IS NULL OR next_execution_time <= $3) AND time_taken IS NULL", 
    [t, userId, t])
        .then(function(result){
            return result;
        })
...
UserDAO.prototype.unlockUserForDailyUpdate = function(userId) {
    var t = navCommonUtil().getMillisecondTill(18);
    return this.dbQuery("UPDATE "+ tableName+" SET time_taken = NULL, next_execution_time = $1 WHERE _id=$2", 
    [new navCommonUtil().getCurrentTime() + t, userId])
        .then(function(result){
            return result;
        })
        .catch(function (error) {
...
UserDAO.prototype.updateTimeForNextDailyUpdate =function(updateTime) {
