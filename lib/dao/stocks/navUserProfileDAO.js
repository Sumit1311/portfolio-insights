...
navUserProfileDAO.prototype.checkIfEligibleForProfileRefresh = function(userId) {
    ...
    return self.dbQuery("SELECT DISTINCT user_id, trxn_date FROM "+tableName+" " +
        "WHERE trxn_flag = 0 AND user_id = $1 AND" +
        " is_active = 1 AND is_completed = 0" +
...
navUserProfileDAO.prototype.getViewProfileData = function(userId) {
    var self = this;
    return self.dbQuery("select u.*, s.security_id, s.security_name from user_stocks_trxn u INNER JOIN stock_list_bse s ON u.security_code = s.security_code WHERE u.user_id = $1 AND u.is_active = 1", [userId] )
        .then(function (result) {
            return result.rows;
        })
        .catch(function (error) {
