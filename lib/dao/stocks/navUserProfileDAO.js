...
navUserProfileDAO.prototype.getTransaction = function(transactionId) {
    ...
    return return self.dbQuery("select * " +
...
navUserProfileDAO.prototype.deleteTransaction = function(transactionId) {
    var self = this;
    return self.dbQuery("DELETE " +
        " FROM user_stocks_trxn " +
        " WHERE _id = $1 AND is_active = 1 AND trxn_flag = 0", [transactionId] )
        .then(function (result) {
            return result.rowCount;
        })
        .catch(function (error) {
...
navUserProfileDAO.prototype.markTransactionInactive = function(transactionId) {
    var self = this;
    return self.dbQuery("UPDATE user_stocks_trxn" +
        ...
        [transactionId , navCommonUtil.getCurrentTime_S()] )
        .then(function (result) {
            return result.rowCount;
        })
        .catch(function (error) {
...
navUserProfileDAO.prototype.unlockRow = function(transactionId){
    ...
    return self.dbQuery("UPDATE user_stocks_trxn SET time_taken=NULL WHERE time_taken IS NOT NULL AND _id = $2",
        [transactionId] )
        .then(function (result) {
            return result.rowCount;
        })
...
navUserProfileDAO.prototype.undoTransaction = function(transactionId, userId) {
    var self = this;
    return self.dbQuery("DELETE " +
        " FROM user_stocks_trxn " +
