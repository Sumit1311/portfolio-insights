...
navUserProfileDAO.prototype.getTransaction = function(transactionId, userId) {
    var self = this;
    return self.dbQuery("select * " +
        " FROM user_stocks_trxn " +
        " WHERE u.user_id = $1 AND _id = $2 AND is_active = 1", [userId, transactionId] )
        .then(function (result) {
            return result.rows;
        })
        .catch(function (error) {
...
navUserProfileDAO.prototype.deleteTransaction = function(transactionId, userId) {
    var self = this;
    return self.dbQuery("DELETE " +
        " FROM user_stocks_trxn " +
        " WHERE u.user_id = $1 AND _id = $2 AND is_active = 1 AND trxn_flag = 0", [userId, transactionId] )
        .then(function (result) {
...
navUserProfileDAO.prototype.markTransactionInactive = function(transactionId, userId) {
    var self = this;
    "UPDATE user_stocks_trxn SET is_active = 0, trxn_flag = 0, deactivation_date = $1, parent_id = $2" +
    " WHERE user_id = $3 AND (SELECT count(_id) FROM user_stocks_trxn WHERE u.user_id = $3 AND parent_id = $2 AND is_active = 0 AND trxn_flag = 0) == 0"
    return self.dbQuery("SELECT * " +
        " FROM user_stocks_trxn " +
        " WHERE u.user_id = $1 AND parent_id = $2 AND is_active = 0 AND trxn_flag = 0", [userId, transactionId] )
        .then(function (result) {
            return result.rows;
        })
        .catch(function (error) {
